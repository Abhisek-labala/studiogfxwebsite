...
const DEFAULT_CONFIG = {
  ...
  aboutYears: "12+",
  contactTitle: "Start Your Legacy",
  contactDescription: "We document weddings globally. Dates are highly limited to preserve our fine-art post-processing standards. Share your wedding vision and let's build beautiful visual coven memoirs together.",
  packages: [
    {
      name: "The Elopement",
...
export const db = {
  ...
  // Config Operations
  async getConfig() {
    const data = await this.read('config', DEFAULT_CONFIG);
  ...
  async updateGalleryItem(id, updatedFields) {
    ...
    gallery[index] = { ...gallery[index], ...updatedFields };
    await this.write('gallery', gallery);
